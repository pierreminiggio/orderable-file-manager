(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["OrderableFileManager"] = factory();
	else
		root["OrderableFileManager"] = factory();
...
const BinaryUrlReader = __webpack_require__(3);
...
class OrderableFileManager {
	...
	loadNewList() {
		...
		let fileKeys = Object.keys(this.files);

		for (let i = 0; i < fileKeys.length; i++) {
			if (!newIds.includes(this.files[fileKeys[i]].file.id)) {
				this.files[fileKeys[i]].element.remove();
				delete this.files[fileKeys[i]];
			}
		}
	}
	...
	async loadFileThumb(file, field, querySelector) {
		...
		BinaryUrlReader.read(file[field], reader => {
...
const OrderableFileManager = __webpack_require__(4);
